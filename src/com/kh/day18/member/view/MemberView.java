...
public class MemberView {
	...
	public void startMemberProgram() {
		...
		while (true) {
			...
			case 2:
				...
				mOne = controller.findOneByName(name);		// 기존 저장된 데이터
				if(index != -1) {
					mOne = this.modifyMembers(mOne);  // 데이터가 있을때 수정할 정보를 입력 받음
					// 수정된 데이터(mOne)
				}
			...
			case 3:
				...
				index = controller.findIndexByName(name);
				if(index != -1) {
					controller.subMember(index);				//데이터 삭제					
				}
				break;
			case 4:
	...
	public Member modifyMembers(Member member) {
			Scanner sc = new Scanner(System.in);

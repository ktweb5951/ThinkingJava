...
public class Exercise_RadioButtonItemEvent extends JFrame {
	...
	private JLabel imageLabel;							// 이미지라벨 선언
	...
	public Exercise_RadioButtonItemEvent() {
		...
		JPanel radioPanel = new JPanel(); 			//라디오 버튼을 추가할 펜을 만듬(이후 라디오팬이라고 함)
		...
		for(int i=0; i<fruits.length;i++) {         // 반복문으로 라디오 버튼 객체 생성해서 배열에 넣고
			fruits[i] = new JRadioButton(names[i]); // 새로 만든 라디오 팬에 추가함.
			btnG.add(fruits[i]); 					// 라디오 버튼 중 1개만 선택되도록 버튼 그룹 섷정해줌
...
//					}
					if(fruits[0].isSelected()) {
						imageLabel.setIcon(images[0]); // setIcon() 메소드 처음본다. Label 이미지를 set해주는 매소드

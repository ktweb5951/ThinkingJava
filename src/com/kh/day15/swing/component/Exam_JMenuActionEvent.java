...
public class Exam_JMenuActionEvent extends JFrame{
	...
	public Exam_JMenuActionEvent() {
		setTitle("메뉴에 리스너 만들기");
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		createMenu();
		Container c = getContentPane();  //컨텐츠펜
		imgLabel = new  JLabel();		 //이미지 들어갈 라벨
		c.setLayout(new BorderLayout()); // BorderLayout 사용 , (Top, BOTTM,LEFT, RIGHT, GENTER) ;
		...
		for(int i=0; i<4; i++) {
			...
			menuItem.addActionListener(new ActionListener() {	//아이템이 선택되었을때
				...
				public void actionPerformed(ActionEvent e) {
					...
					switch(cmd) { // else if 보다 빠른 swtich case문
						case "Load" :
